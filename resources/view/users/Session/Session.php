    ...
    <div class="flex-1 bg-white rounded-2xl shadow p-6">
        ...
        <div id="seccion-editable" class="hidden">
            ...
            <div class="flex justify-end mt-3 mr-1">
                <button onclick="guardarContenido()" class="btn btn-primary text-sm px-5 py-2">
                    Guardar contenido
                </button>
            </div>
        </div>
    ...
    <div style="background:white; border-radius:16px; width:100%; max-width:580px; box-shadow:0 8px 32px rgba(0,0,0,0.18); overflow:hidden;">
        ...
        <div style="background:var(--granate-700); padding:14px 20px; display:flex; justify-content:space-between; align-items:center;">
            <div style="display:flex; align-items:center; gap:8px;">
                ...
                <span style="color:white; font-family:'Playfair Display',serif; font-size:15px;">Crear evaluación</span>
            </div>
            <button onclick="cerrarModalEvaluacion()"
                    style="color:white; background:none; border:none; font-size:18px; cursor:pointer; opacity:.8;"
        ...
        <div style="padding:24px 24px 20px;">
            ...
            <div style="display:flex; align-items:center; gap:10px; margin-bottom:16px;">
                ...
                <span style="font-size:11px; color:var(--granate-400); font-weight:600; letter-spacing:.06em; text-transform:uppercase;">Sección de preguntas</span>
                <div style="flex:1; height:1px; background:var(--granate-100);"></div>
            </div>

            <div id="filas-tipos" style="display:flex; flex-direction:column; gap:10px; margin-bottom:14px;"></div>

            <button type="button" onclick="agregarFilaTipo()" id="btn-agregar-tipo"
                    style="font-size:12px; color:var(--granate-500); background:none; border:none; cursor:pointer; padding:0; margin-bottom:20px;"
            ...
            <div style="display:flex; justify-content:flex-end; gap:10px;">
                <button onclick="abrirImportarXML()"
                        style="background:var(--granate-100); color:var(--granate-600); border:none; border-radius:8px; padding:8px 16px; font-size:13px; font-weight:600; cursor:pointer;"
                        onmouseover="this.style.background='var(--granate-200)'"
                        onmouseout="this.style.background='var(--granate-100)'">Importar XML</button>
                <button onclick="guardarConfigEvaluacion()"
                        style="background:var(--granate-600); color:white; border:none; border-radius:8px; padding:8px 20px; font-size:13px; font-weight:600; cursor:pointer;"
                        onmouseover="this.style.background='var(--granate-700)'"
                        onmouseout="this.style.background='var(--granate-600)'">Guardar</button>
                <button onclick="cerrarModalEvaluacion()"
                        style="background:var(--fondo-card); color:var(--texto-secundario); border:none; border-radius:8px; padding:8px 16px; font-size:13px; font-weight:600; cursor:pointer;"
                        onmouseover="this.style.background='var(--fondo-hover)'"
                        onmouseout="this.style.background='var(--fondo-card)'">Cancelar</button>
            </div>
        </div>
    </div>
</div>
